...
//    @PostConstruct
    ...
    private void first() {
        ...
        System.out.println("澳门线程启动："+new Date().toString());
        TbDrawRecord lastRecord = tbDrawRecordService.selectLastTbDrawRecordByGameType(1);
        if(lastRecord != null && lastRecord.getResultStatus()!=null && lastRecord.getResultStatus()!=3){
            ...
            while (record == null){
                ...
                count++;
                record = DrawClientUtils.fetchAm(lastRecord.getPeriodNumber());
                if (record==null){
                    continue;
                }
        ...
        record = DrawClientUtils.fetchAmLast();
        if (record!=null){
            ...
            Global.amCurrentDraw = nextRecord;
            Global.amCurrentNo = record.getNextPeriodNumber();
            Global.amOpenDate = record.getNextOpenTime();
        }
    ...
    public TbDrawRecord performDrawLogic() {
        // 替换为实际业务逻辑
        System.out.println("澳门开奖线程："+new Date().toString());
        //更新当期开奖结果
        ...
        try{
            Thread.sleep(3000);
            if(!StringUtils.isEmpty(Global.amCurrentNo)){
                tbDrawRecord = tbDrawRecordService.selectTbDrawRecordByNo(Global.amCurrentNo);
            }
            ...
            while (true){
                ...
                if(!alreadyDraw){
                    record = DrawClientUtils.fetchAm(Global.amCurrentNo);
                }else{
                    record = DrawClientUtils.fetchAmLast();
                }
                ...
                if(!alreadyDraw){
                    ...
                    tbDrawRecordService.updateTbDrawRecord(tbDrawRecord);
                    Global.amCurrentNo = null;
                    Global.amOpenDate = null;
                ...
                for (TbRoomConfig config:configList) {
                    ...
                    if(!config.getRoomName().contains("澳门")){
                        continue;
                    }
                    roomConfigMap.put(config.getId(),config);
                ...
                for (Long key :scoreMap.keySet()) {
                    ...
                    for(int i = 0;i<temp.size();i++){
                        sb.append(temp.get(i).getUserName().substring(0,2)+":"+temp.get(i).getWinScore().doubleValue()+space);
                        if(i%3==2){
                            sb.append(lineEnd);
                        }
                    }
    ...
    public TbDrawRecord performPauseLogic() {
        // 替换为实际业务逻辑
        System.out.println("澳门封盘线程："+new Date().toString());
        TbDrawRecord tbDrawRecord = tbDrawRecordService.selectTbDrawRecordByNo(Global.amCurrentNo);
        Global.amCurrentDraw.setResultStatus(1);
        ...
        for (Long key :scoreMap.keySet()) {
            ...
            for(int i = 0;i<temp.size();i++){
                sb.append(temp.get(i).getUserName()+space+"投："+temp.get(i).getBetting()+space+"余："+temp.get(i).getLeftScore().doubleValue()+lineEnd);
            }
            sb.append("<span class='content_close'>- - - - - - - - - - - - - - - -  - - - -  - - - - - - - - - - - - - - - -</span>"+lineEnd);
