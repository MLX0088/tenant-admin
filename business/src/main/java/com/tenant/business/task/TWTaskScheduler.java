...
public class TWTaskScheduler {
    ...
    private void first() {
        ...
        System.out.println("台湾线程启动："+new Date().toString());
        TbDrawRecord lastRecord = tbDrawRecordService.selectLastTbDrawRecordByGameType(2);
        if(lastRecord != null && lastRecord.getResultStatus()!=null && lastRecord.getResultStatus()!=3){
            ...
            while (record == null){
                ...
                count++;
                record = DrawClientUtils.fetchTw(lastRecord.getPeriodNumber());
                if (record==null){
                    continue;
                }
                ...
                for (TbScoreRecord score : scoreList ) {
                    ...
                    double diff = score.getWinScore().longValue()>0 ? score.getWinScore().longValue() : score.getScore().add(score.getWinScore()).doubleValue();
                    if(diff>0){
        ...
        record = DrawClientUtils.fetchTwLast();
        if (record!=null){
    ...
    public TbDrawRecord performDrawLogic() {
        // 替换为实际业务逻辑
        System.out.println("台湾开奖线程："+new Date().toString());
        //更新当期开奖结果
        ...
        try{
            Thread.sleep(3000);
            if(!StringUtils.isEmpty(Global.twCurrentNo)){
                tbDrawRecord = tbDrawRecordService.selectTbDrawRecordByNo(Global.twCurrentNo);
            }
            ...
            while (true){
                ...
                if(!alreadyDraw){
                    record = DrawClientUtils.fetchTw(Global.twCurrentNo);
                }else{
                    record = DrawClientUtils.fetchTwLast();
                }
                ...
                if(record==null){
                    ...
                    if(!alreadyDraw &&  nowTime - tbDrawRecord.getExpectOpenTime().getTime() > 1000*60*60*2){
                        if(Global.twIsOpen){
                            record = DrawClientUtils.fetchTwLast();
                            if(!record.getNextPeriodNumber().equals(Global.twCurrentNo)){
                                //新增下期开奖结果
                ...
                if(!alreadyDraw){
                    ...
                    tbDrawRecordService.updateTbDrawRecord(tbDrawRecord);
                    Global.twCurrentNo = null;
                    Global.twOpenDate = null;
                ...
                if(Global.twDrawList.size()<=0 || Integer.parseInt(Global.twDrawList.get(0).getPeriodNumber())<Integer.parseInt(record.getPeriodNumber())){
                    Global.twDrawList.add(0,record);
                    if(Global.twDrawList.size()>10){
                        Global.twDrawList = new ArrayList<>(Global.twDrawList.subList(0,10));
                    }
                }
                String historyList = "";
                ...
                for (TbRoomConfig config:configList) {
                    ...
                    if(!config.getRoomName().contains("台湾")){
                        continue;
                    }
                    roomConfigMap.put(config.getId(),config);
...
//                        scoreMap.get(score.getRoomConfigId()).add(score.parseTo());
                ...
                List<TbRobot> robotList = tbRobotService.selectRobotForScoreFilter(60);
                ...
                for (Long key :scoreMap.keySet()) {
                    ...
                    for(int i = 0;i<temp.size();i++){
                        ...
                        if (val.length() < 12) {
                            val = String.format("%-12s", val).replace(" ","&nbsp;&nbsp;");
                        }
                        sb.append(val);
                        if(i%3==2){
                            sb.append(lineEnd);
    ...
    public TbDrawRecord performPauseLogic() {
        // 替换为实际业务逻辑
        System.out.println("台湾封盘线程："+new Date().toString());
        TbDrawRecord tbDrawRecord = tbDrawRecordService.selectTbDrawRecordByNo(Global.twCurrentNo);
        Global.twCurrentDraw.setResultStatus(1);
